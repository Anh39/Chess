...
import ai_engine
from old.pieces import Piece,PieceType,Side
...
class Board:
    ...
    def _get_all_board(self):
        piece_sides = [[None for i in range(8)] for j in range(8)]
        piece_types = [[None for i in range(8)] for j in range(8)]
        piece_defense = [[None for i in range(8)] for j in range(8)]
        piece_capture = [[None for i in range(8)] for j in range(8)]
        piece_move = [[None for i in range(8)] for j in range(8)]
        for i in range(8):
            for j in range(8):
                ...
                if (target_piece.side == Side.Up):
                    ...
                    piece_sides[i][j] = 'd'
                piece_types[i][j] = target_piece.to_string_minimum()
                ...
                piece_defense[i][j] = defensable_string
                piece_capture[i][j] = capturable_string
                piece_move[i][j] = moveable
        return (piece_types,piece_sides,piece_move,piece_capture,piece_defense)
    def _get_all_board2(self):
        up_pieces = {}
        down_pieces = {}
        for i in range(8):
            for j in range(8):
